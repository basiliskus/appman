...
def get_file_paths(repo_root, filter=None):
    for path, directories, files in os.walk(repo_root):
        files = [
...
def get_validation_files(schemas_root, repo_root, name):
    schema_path = os.path.join(schemas_root, f"{name}{EXT}")
    data_path = os.path.join(repo_root, name)
